...
class dun163():
    ...
    def getVerifyImgLocationX(self,verifyInfo):
        bg = self.sess.get(verifyInfo['bg']).content
        bg_arr = np.asarray(bytearray(bg),dtype=np.uint8)
        bg_img = cv2.imdecode(bg_arr,-1)

        front = self.sess.get(verifyInfo['front']).content
    ...
    def identify_gap(self,bg,tp,out="out.jpg"):
        ''' 識別滑塊缺口位置,返回x坐標
            bg: 背景图片
            tp: 缺口图片
            out:输出图片
    ...
    def checkCode(self,token,data,cb):
        headers = {
            ...
            'Cache-Control': 'no-cache',
            'Connection': 'keep-alive',
            # 'Cookie': '_ga=GA.1.288ffd5247189.2eaa87d52df9504b4b1a; Hm_lvt_4671c5d502135636b837050ec6d716ce=1680831446; __root_domain_v=.163.com; _qddaz=QD.110080831446904; Hm_lpvt_4671c5d502135636b837050ec6d716ce=1680833869',
            'Pragma': 'no-cache',
            'Referer': 'https://dun.163.com/',
            'Sec-Fetch-Dest': 'script',
            'Sec-Fetch-Mode': 'no-cors',
        ...
        response = self.sess.get('https://c.dun.163.com/api/v3/check', params=params, headers=headers)
        print(response.text)
